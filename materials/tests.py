...
class LessonsAPITests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user_owner = User.objects.create_user(email='owner@example.com', password='pass')
        cls.user_regular = User.objects.create_user(email='regular@example.com', password='pass')

        cls.user_moderator = User.objects.create_user(email='moderator@example.com', password='pass')
    ...
    def test_create_lesson_success(self):
        ...
        response = self.client.post(reverse('materials:lesson-list'), data=lesson_data, format='json')
    ...
    def test_update_own_lesson(self):
        ...
        response = self.client.patch(reverse('materials:lesson-detail', args=[lesson.pk]), data=update_data,
                                     format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class SubscriptionsAPITests(APITestCase):
    ...
    def test_subscription_flow(self):
        ...
        sub_response = self.client.post('/subscription/', data={'course_id': course_id}, format='json')
        ...
        detail_response_after = self.client.get(reverse('materials:course-detail', args=[self.__class__.course_1.id]))
