...
from .serializers import UserSerializer, UserProfileSerializer, UserAvatarUpdateSerializer, RegisterSerializer, PaymentSerializer
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not course_id:
            return Response({'detail': 'Укажите ID курса.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                price_resp = create_stripe_price(course.stripe_product_id, amount_cents)
        ...
        with transaction.atomic():
            ...
            if not course.stripe_product_id:
                ...
                if not prod_response['success']:
                    payment.delete()
                    return JsonResponse({'error': prod_response['error']}, status=400)
            ...
            if not price_response['success']:
                payment.delete()
                return JsonResponse({'error': price_response['error']}, status=400)
            ...
            if not session_response['success']:
                payment.delete()
                return JsonResponse({'error': session_response['error']}, status=400)
